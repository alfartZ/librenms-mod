...
function custom_health_processor(Request $request)
{
    ...
    $processors = dbFetchRows('SELECT * FROM `processors` WHERE device_id = ?', [$device_id]);

    foreach ($processors as $proc) {
        $id = 'id';
        $val = $proc['processor_id'];
        $proc_url = 'graphs/' . $id . '=' . $val . '/type=processor_usage/';
        $base_url = 'graph.php?' . $id . '=' . $val . '&amp;type=processor_usage&amp;from=' . Config::get('time.day') . '&amp;to=' . Config::get('time.now');
        ...
        $proc_popup .= "', RIGHT" . Config::get('overlib_defaults') . ');" onmouseout="return nd();"';
...
function show_graph(Request $request)
{
    ...
    $hostname = $vars['device'];
    // $device = ctype_digit($hostname) ? Device::find($hostname)->toArray() : Device::findByHostname($hostname)->toArray();
    $device = is_numeric($hostname) ? DeviceCache::get((int) $hostname) : DeviceCache::getByHostname($hostname);
...
function trigger_auto_discovery(Request $request)
{
    $vars = [
